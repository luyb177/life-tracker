...
// 创建自定义分类
func CreateExpenseCategoryHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.CreateExpenseCategoryReq
		if err := httpx.Parse(r, &req); err != nil {
			respx.ErrorCtx(r.Context(), w, errorx.WrapBadRequest("请求参数解析失败", err))
			return
		}

		l := expense.NewCreateExpenseCategoryLogic(r.Context(), svcCtx)
		resp, err := l.CreateExpenseCategory(&req)
		if err != nil {
